...
**Goal:** "내가 원하는 에이전트, 직접 만들 수 있다" 세션의 슬라이드 18장 + 각 슬라이드 핵심 내용을 마크다운 문서로 완성한다.
...
```bash
git add docs/session/koog-agent-session.md
git commit -m "docs: 세션 3부 슬라이드 10-14 작성"
```
...
```bash
git add docs/session/koog-agent-session.md
git commit -m "docs: 세션 3부 슬라이드 15-16 작성"
```
