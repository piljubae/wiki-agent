...
**제목:** "직접 만드는 AI 에이전트 — Koog로 배우는 AI 에이전트 설계"
...
| 레벨 | 설명 | 얻어가는 것 |
|------|------|------------|
| AI 입문 | 대화형 LLM만 써봤음 | 에이전트 구성 요소 이해, 설계 방법론 |
| API 경험자 | API 써봤음 | 단순 API 호출과 에이전트의 차이, 프롬프트 설계 원칙 |
| 프레임워크 경험자 | LangChain 등 써봤음 | Koog 아키텍처 비교, Kotlin-native 에이전트 패턴 |

**핵심 takeaway:** 에이전트 설계 방법론 + Koog 이해 + 내 케이스에 적용
...
- 슬라이드 2: 데모 — wiki-agent 실제 동작 시연
  - `@wiki 배포 프로세스 알려줘` → `:mag: Confluence 검색 중...` 중간 메시지 → 삭제 → 최종 답변 + `📋 Confluence 3건` 출처 footer
  - ReAct(추론→행동→관찰) 과정이 실시간으로 보임
  - GitHub Wiki, RAG 활성화 시 복수 Tool 선택 과정 시각화
- 슬라이드 3: "Koog으로 만드는 에이전트"
...
- 슬라이드 7: Provider 교체 한 줄 — 다양한 API 및 환경 설정 가능
- 슬라이드 8: Orchestrator + Specialist 패턴
  - 설계 다이어그램 → 실제 코드로 연결
  - `ToolRegistry { tool(confluenceTool::confluenceSearch) }` 한 줄이 Tool 등록의 전부
  - GitHub Wiki 활성화: `tool(githubWikiTool::githubWikiSearch)` 한 줄 추가
...
- 슬라이드 15: 실서비스 운영 — 모니터링, 대화 기록, 메모리
  - EventHandler: Tool 선택 과정 실시간 모니터링 (ReAct 패턴)
  - 대화 기록: 스레드별 JSONL 저장 → prompt DSL로 맥락 주입 (최근 5턴)
  - Sliding Window: 10턴 초과 시 LLM 요약으로 토큰 절약
  - 프로젝트 메모리: `/wiki memory add`로 프로젝트 특성 저장 → 검색 품질 향상
- 슬라이드 16: 직군별 에이전트 아이디어 (PM/HR/개발자/마케터) — wiki-agent 확장 가능성
  - GitHub Wiki Tool 추가 예시: 기술 레포 Wiki 연결로 개발팀 온보딩 자동화
- 슬라이드 17: 워크숍 — wiki-agent 설정 → Slack에서 직접 질문 → 결과 확인 (35분)
  - 기본: `.env.example` 복사 → 토큰 입력 → `./gradlew run` → `@wiki` 질문 → 중간 메시지 + footer 확인
  - `/wiki memory add` 명령 체험
