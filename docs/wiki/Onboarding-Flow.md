...
> DM에서 봇에게 처음 메시지를 보내면 왜 설정 질문이 나오나요?

## 개요

봇에게 프로젝트 정보가 없을 때 (`.wiki/memory.md`가 없을 때) DM 첫 메시지에서 4단계 온보딩이 시작됩니다.  
...
이후 DM 메시지는 바로 검색으로 처리됩니다. 저장된 정보는 `/wiki memory show`로 확인할 수 있습니다.

## 온보딩 건너뛰기

`/wiki memory add <내용>`으로 직접 정보를 저장하면 온보딩이 트리거되지 않습니다.
