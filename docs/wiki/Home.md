...
배필주2(wiki-agent)는 Confluence · GitHub Wiki · ChromaDB(RAG) 에서 관련 문서를 찾아 답변하는 Slack AI 어시스턴트입니다.  
Slack AI 패널에서 직접 질문하거나, 채널에서 `@배필주2 질문`으로 검색할 수 있습니다.  
...
| [Onboarding-Flow](Onboarding-Flow) | 온보딩 마법사는 어떻게 동작하나요? |
...
| [Google-Embedding-API](Google-Embedding-API) | Google 임베딩 모델 선택·엔드포인트·키 분리는? |
...
| [Incremental-Indexing](Incremental-Indexing) | 변경된 파일만 재인덱싱하는 원리는? |
| [Parallel-Embedding](Parallel-Embedding) | 임베딩을 병렬로 실행하는 방법은? Semaphore는 무엇인가요? |
| [Embedding-Cache](Embedding-Cache) | 이미 인덱싱된 청크를 어떻게 스킵하나요? |
