...
Gradle 멀티모듈로 분리 완료 ([모듈화 로드맵](docs/plans/2026-06-09-modularization-roadmap.md)). 의존성은 안쪽(공통 기반)에서 바깥(앱)으로 **단방향**으로만 흐른다(순환 0).

| 모듈 | 책임 | README |
|------|------|--------|
| `(root) :app` | Slack 게이트웨이(`slack`) · 오케스트레이터(`agent`) · `Main` 조립 (Composition Root) | — |
| `:knowledge` | 지식베이스 구축·인덱싱(코드/PR/콜그래프)·검색 | [README](knowledge/README.md) |
| `:onboarding` | 신규 입사자 온보딩 커리큘럼 진행 | [README](onboarding/README.md) |
| `:search` | 검색/도구 레이어(Tool·검색 에이전트·BM25Index) | [README](search/README.md) |
| `:llm` | provider별 LLM 실행기 조립 | [README](llm/README.md) |
| `:confluence` | Confluence REST 클라이언트 | [README](confluence/README.md) |
| `:github` | GitHub REST 클라이언트(코드·PR·위키) | [README](github/README.md) |
| `:rag` | 임베딩 + ChromaDB 벡터 검색 | [README](rag/README.md) |
...
대략의 의존 방향: `:app → :knowledge·:onboarding·:search → :confluence·:github·:rag → :config·:context`. 모듈의 public API에 노출되는 의존은 `api`로, 내부 전용은 `implementation`으로 선언한다.
