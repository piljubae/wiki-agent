...
class AntigravityCodeLLMClient(
    private val antigravityPath: String = DEFAULT_ANTIGRAVITY_PATH,
    ...
    private suspend fun runProcess(prompt: String, modelId: String): String = withContext(Dispatchers.IO) {
        ...
        val process = try {
            ...
            throw IllegalStateException("Failed to start antigravity binary: ${e.message}", e)
        }
        ...
        if (!completed) {
            ...
            throw TimeoutException("antigravity CLI timed out after ${timeoutSeconds}s")
        }
        val output = process.inputStream.bufferedReader().readText()
        val exitCode = process.exitValue()
        if (exitCode != 0) throw IllegalStateException("antigravity CLI exited with code $exitCode: ${output.take(300)}")
        output.trim()
    }

    companion object {
        private const val DEFAULT_ANTIGRAVITY_PATH = "agy"
        private const val TIMEOUT_SECONDS = 120L
        private val SEARCH_REGEX = Regex("^SEARCH:\\s*(.+)$", RegexOption.MULTILINE)
        private val log = LoggerFactory.getLogger(AntigravityCodeLLMClient::class.java)
    }
}
